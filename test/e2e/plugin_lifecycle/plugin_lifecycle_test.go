...
const PluginInstalledRegExp = "Installed plugin '%s:.+' with target '%s'|Reinitialized plugin '%s:.+' with target '%s'"
const PluginInstalledMsg = "successfully installed '%s' plugin"
	...
	Context("plugin use cases: tanzu plugin clean, install and describe, list, delete", func() {
		...
		It("install plugins and describe each installed plugin", func() {
			for _, plugin := range util.PluginsForLifeCycleTests {
				...
				Expect(stdErr).To(ContainSubstring(fmt.Sprintf(PluginInstalledMsg, plugin.Name)))
