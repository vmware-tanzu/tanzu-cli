...
func TestConfigFeaturesDefaultEditionAdded(t *testing.T) {
	...
	errMsg := "addDefaultEditionIfMissing should have added default edition (" + configtypes.EditionStandard + ") instead of " + cfg.ClientOptions.CLI.Edition
	require.Equal(t, cfg.ClientOptions.CLI.Edition, configtypes.EditionSelector(configtypes.EditionStandard), errMsg)
}
...
func TestConfigFeaturesDefaultEditionNotAdded(t *testing.T) {
	...
	require.Equal(t, cfg.ClientOptions.CLI.Edition, configtypes.EditionSelector(configtypes.EditionCommunity), errMsg)
}
