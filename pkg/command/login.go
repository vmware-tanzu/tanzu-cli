...
func init() {
	...
	// TODO(prkalle): update the Note to point to login with TANZU_API_TOKEN and interactively
	loginCmd.Example = `
    # Login to Tanzu
    tanzu login

    # Login to Tanzu using endpoint
...
func updateKubeConfigForContext(c *configtypes.Context) error {
	projectNameStr := getString(c.AdditionalMetadata[config.ProjectNameKey])
	...
	return updateTanzuContextKubeconfig(c, projectNameStr, spaceNameStr, clusterGroupNameNameStr)
}
