...
<<<<<<< HEAD
	...
	log.Success(msg)
=======
>>>>>>> 3c31796b (context create changes)
	return nil
}
...
func useCtx(cmd *cobra.Command, args []string) error {
	...
	_, err = syncContextPlugins(cmd, ctx.ContextType, ctxName, true)
	if err == nil {
		log.Infof("%s and required plugins are up-to-date", ctxCreateMsg)
	} else {
		log.Warningf("%s but unable to sync the plugins. Please run 'tanzu plugin sync' command to sync plugins manually, error: '%v'", ctxCreateMsg, err.Error())
	}
	return err
}
