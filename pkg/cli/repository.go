...
func (g *GCPBucketRepository) Describe(name string) (plugin Plugin, err error) {
	...
	pluginPath := path.Join(g.rootPath, name, PluginDescriptorFileName)
	...
	for {
		...
		if version != PluginDescriptorFileName {
			versionMap[version] = ""
		}
	}
	versions := []string{}
...
func (l *LocalRepository) Describe(name string) (plugin Plugin, err error) {
	b, err := os.ReadFile(filepath.Join(l.path, name, PluginDescriptorFileName))
	if err != nil {
