...
func TestCatalogSuite(t *testing.T) {
	...
	RunSpecs(t, "plugin catalog test suite")
}
...
var _ = Describe("GetInstalledPlugins (standalone plugins)", func() {
	...
	Context("when no standalone or server plugins installed", func() {
		...
		It("should return empty plugin list", func() {
			MigrateContextPluginsAsStandaloneIfNeeded()
...
func fakeInstallPlugin(contextName, pluginName string, target types.Target, version string) (*cli.PluginInfo, error) {
	cc, err := NewContextCatalogUpdater(contextName)
	if err != nil {
		return nil, err
	}
	defer cc.Unlock()
...
func getInstalledPlugins() ([]cli.PluginInfo, error) {
	...
	standAloneCatalog, err := NewContextCatalog("")
	if err != nil {
		return nil, err
	}
...
func isPluginInstalled(name string, target types.Target, version string) bool {
