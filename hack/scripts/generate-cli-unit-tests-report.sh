...
# Extract each test suite test cases count, failed test cases count and the package name
testsuites=$(echo "$junit_report_xml" | sed -n 's/.*<testsuite tests="\([^"]*\)" failures="\([^"]*\)" .*name="\([^"]*\)".*/\1 \2 \3/p')

# To Calculate total test cases count, passed test cases count and failed test cases count.
total_tests_count=0
total_passed_count=0
total_failed_count=0

# To Calculate average coverage percentage
total_coverage_percentage=0
total_packages=0

# Process each testsuite, and check if any ginkgo test cases available, and also code coverage for each package ($name)
    ...
    while IFS= read -r line; do
      ...
      if [[ "$line" == "$coverageInfo,"* ]]; then
          ...
          if [ -n "$coverage_val" ]; then
              ...
              coverage_percentage_val=$(echo "$coveragePercentage" | grep -oE '[0-9.]+%' | sed 's/%//g')
              # Calculate the total coverage percentage and the number of packages
              total_coverage_percentage=$(echo "$total_coverage_percentage + $coverage_percentage_val" | bc)
              ((total_packages++))
          fi
          break
      fi
...
average_coverage_percentage=$(echo "scale=2; $total_coverage_percentage / $total_packages" | bc)

# Print the total line with color and icon depending on the result
if [ "$total_failed_count" -eq 0 ]; then
  echo "| **Total** | **$total_tests_count** | **$total_passed_count** | **$total_failed_count** | **$average_coverage_percentage** |"
else
  echo "| **Total** | **$total_tests_count** | **$total_passed_count** | :x: **$total_failed_count** | **$average_coverage_percentage** |"
fi